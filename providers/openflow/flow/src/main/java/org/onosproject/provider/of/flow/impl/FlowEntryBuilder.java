    ...
    private TrafficSelector buildSelector() {
        ...
        for (MatchField<?> field : match.getMatchFields()) {
            ...
            case IPV6_EXTHDR:
                ...
                break;
            case OCH_SIGID:
                log.warn("Hard-coding OCH_SIGID with GRID_TYPE=DWDM and CHANNEL_SPACING=50GHz");
                ...
                builder.add(matchLambda(Lambda.ochSignal(
                        lookupGridType((byte) 1), lookupChannelSpacing((byte) 2),
                        sigId.getChannelNumber(), (short) 1)
                ));
                break;
            case OCH_SIGTYPE:
            ...
            case EXP_OCH_SIG_ID:
                try {
                    ...
                    builder.add(matchLambda(Lambda.ochSignal(
                            lookupGridType((byte) 1), lookupChannelSpacing((byte) 2),
                            expSigId.getChannelNumber(), (short) 1)
                    ));
                } catch (NoMappingFoundException e) {
                    log.warn(e.getMessage());
                    break;
