...
//                    cancelTask();
                    initLinkSet();
//                    sendMouseData();
                    sendLinkData(VnoId.vnoId(1));
                    break;

                case "vno2":
                    log.debug("Mode VNO 2");
                    currentMode = Mode.LINK;
                    //scheduleTask();
                    initLinkSet();
                    sendLinkData(VnoId.vnoId(2));
                    break;
...
//                sendLinkDataVno2();
            ...
            case LINK:
                log.debug("case LINK on updateForMode");
//                sendLinkData();
                break;
    ...
    private Set<Link> getLinksByVno(VnoId vnoId, Link[] linkSet) {
        Set<Link> vnoLinks = new HashSet<>();

        try {
            NetworkConfigService service = services.get(NetworkConfigService.class);

//        log.warn("2>> Iterating subject classes");
//        for (Class c : service.getSubjectClasses()) {
//            log.warn("Subject class: " + c.getName());
//        }

            SubjectFactory sf = service.getSubjectFactory(LinkKey.class);
//            log.debug("Subject Class Key for LinkKey: " + sf.subjectClassKey());
//            log.debug("Subject Key for LinkKey: " + sf.subjectKey(LinkKey.linkKey(link)));
            for (Link link : linkSet) {
                BasicLinkConfig blc = service.getConfig(LinkKey.linkKey(link), BasicLinkConfig.class);
                if (blc != null) {
                    if (blc.vnoIds() != null) {
                        for (VnoId vno : blc.vnoIds()) {
                            log.debug(String.format("Link [%s] belongs to VNO #%d", LinkKey.linkKey(link).toString(), vno.id()));
                            if (vnoId.id() == vno.id())
                                vnoLinks.add(link);
                        }
                    }
    ...
    private void sendLinkData(VnoId vnoId) {
        clearForMode();
        DemoLinkMap linkMap = new DemoLinkMap();
        Set<Link> vnoLinks = getLinksByVno(vnoId, linkSet);

        for (Link link : vnoLinks) {
            DemoLink dl = linkMap.add(link);
            dl.makeImportant().setLabel(String.format("vno%d", vnoId.id()));
